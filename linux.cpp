...
static void markError(const char* message)
{
    if (message != NULL)
    {
        perror(message);
...
static void handleReadCompletion(AioOperation* readOp, ssize_t bytesRead)
{
    AioOperation* writeOp = (AioOperation*)readOp->next_operation;
    ...
    // после чтения запускаем запись этого же блока
    if (startWrite(writeOp, readOp, readOp->buffer, (size_t)bytesRead, readOp->aio.aio_offset) == -1)
    {
        hasError = 1;
        finishIfDone();
    }
...
static void handleWriteCompletion(AioOperation* writeOp, ssize_t bytesWritten)
{
    size_t requestedBytes = (size_t)writeOp->aio.aio_nbytes;
    AioOperation* readOp = (AioOperation*)writeOp->next_operation;

    // текущая операция записи завершилась
    atomicAdd64(&pendingOperations, -1);

    // если записали не всё, дозаписываем остаток
    if ((size_t)bytesWritten < requestedBytes)
    {
        char* restBuffer = (char*)writeOp->aio.aio_buf + bytesWritten;
        size_t restBytes = requestedBytes - (size_t)bytesWritten;
        off_t restOffset = writeOp->aio.aio_offset + bytesWritten;

        if (startWrite(writeOp, readOp, restBuffer, restBytes, restOffset) == -1)
        {
            hasError = 1;
            finishIfDone();
        }
    ...
    // полная запись блока завершена
    atomicAdd64(&totalWritten, bytesWritten);

    // теперь этим же "слотом" можно запустить следующее чтение
    if (!hasError && readOp != NULL)
    {
        if (submitNextRead(readOp, writeOp) == -1)
        {
            hasError = 1;
        }
    }

    if ((atomicLoad64(&totalWritten) >= (int64_t)fileSize && atomicLoad64(&pendingOperations) == 0) || hasError)
    {
        finishIfDone();
    }
}
...
static void aio_completion_handler(sigval_t sigval)
{
    ...
    {
        handleWriteCompletion(currentOp, result);
    }
    else
    {
        handleReadCompletion(currentOp, result);
    }
}
